...
int InputCallback(ImGuiInputTextCallbackData* data)
{
    ...
    case ImGuiInputTextFlags_CallbackCharFilter:
        if (state.isPopupOpen == false && data->EventChar == '.') {
            state.isPopupOpen = true;
        }
...
void DrawWindow(State& state, ImVec2& popupPos, ImVec2& popupSize, bool& isFocused)
{
    ...
    if (!ImGui::Begin("Query", nullptr, winFlags))
    {
        ImGui::End();
        return;
    }
    ...
    /// Draw any window content here
    {
        ...
        auto stringCol = new ImVec4(0.8, 0.6, 0.4, 1.0);
        auto numberCol = new ImVec4(0.4, 0.6, 0.8, 1.0);
        ...
        if (ImGui::TreeNode("RootNode")) {
            ...
            for (int i = 0; i < 3; i++) {
                if (ImGui::TreeNode((void*)(intptr_t)i, "[%d]", i))
                {
                    ImGui::Text("textKey:");
                    ImGui::SameLine();
                    ImGui::TextColored(*stringCol, "\"this is like, string content or whatever\"");

                    ImGui::Text("numberKey:");
                    ImGui::SameLine();
                    ImGui::TextColored(*numberCol, "420");
